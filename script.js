...
async function getweatherinformation(city) {
    ...
    let getinfo = await fetch(FULL_URL).then(response => response.json()).catch(err => console.error(err))
...
async function searchdata() {
    ...
    try {
        ...
        return await getweatherinformation(city)
    } catch(error) {
        weatherinfo[0].innerText = "Wrong Input Possible"
        weatherinfo[1].innerText = "Failed To Retrieve Weather Information"
        weatherinfo[2].innerText = "No Data Found"
        weatherinfo[3].innerText = "No Data Found"
        weatherinfo[4].innerText = "No Data Found"
        console.error(error)
    }
}